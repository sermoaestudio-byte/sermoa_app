      ...
      <header className="fixed top-0 left-0 right-0 bg-white/90 backdrop-blur-md z-50 border-b border-slate-100 transition-all">
        <div className="max-w-7xl mx-auto px-6 h-20 flex items-center justify-between">
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.scrollTo(0, 0)}>
            {studio.logo_url ? (
              <img src={studio.logo_url} alt={studio.name} className="h-20 object-contain" />
            ) : (
              <span className="text-2xl font-black tracking-tight text-slate-900">{studio.name}</span>
            )}
          </div>
